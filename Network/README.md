...
解决如何连接各种计算机的传输媒体上传输比特流,而不是指具体的传输媒体,制定接口的机械、电气、功能、过程特性标准;
...
* 网线: 直通线(交换机与计算机等不同类),交叉线(计算机与计算机等同类),;
...
* ARP协议(数据发送之前使用)：通过MAC广播将查询IP地址发送，网络中所有电脑对比自己的ip与接收ip，如果相同，则将自己的MAC告诉发送者，从而解析出IP地址对应的MAC地址。
...
* ICMP协议：ping使用的ICMP协议。TTL（windows一般为128，Linux一般为64，Unix一般为255）数据包生存时间，`ping <IP> -i 1`返回与IP网络连接的第一个ip节点.
